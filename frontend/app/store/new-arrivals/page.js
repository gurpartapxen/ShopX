...
function NewArrivalsContent() {
    ...
    const fetchProducts = async (cat) => {
        setLoading(true);
        try {
            const params = { sort: "created_at", order: "desc", limit: 40 };
        ...
        <>
            <style>{`
                ...
                *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .hero { background: linear-gradient(180deg, #111 0%, #0a0a0a 100%); padding: 64px 24px; text-align: center; border-bottom: 1px solid rgba(255,255,255,0.06); position: relative; overflow: hidden; }
                .hero::before { content: ''; position: absolute; top: -80px; left: 50%; transform: translateX(-50%); width: 600px; height: 300px; background: radial-gradient(ellipse, rgba(77,163,255,0.08) 0%, transparent 70%); }
                .hero-eyebrow { font-size: 11px; font-weight: 600; color: #4da3ff; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 14px; position: relative; }
                ...
                .filters { background: #0d0d0d; border-bottom: 1px solid rgba(255,255,255,0.06); padding: 0 24px; }
                .filters-inner { max-width: 1200px; margin: 0 auto; display: flex; gap: 8px; padding: 12px 0; overflow-x: auto; scrollbar-width: none; }
                .filters-inner::-webkit-scrollbar { display: none; }
                .filter-pill { padding: 7px 16px; border-radius: 980px; font-size: 13px; font-weight: 500; cursor: pointer; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.04); color: rgba(255,255,255,0.45); font-family: 'Inter', sans-serif; transition: all 0.15s; white-space: nowrap; }
                .filter-pill:hover { background: rgba(255,255,255,0.08); color: #f5f5f7; }
                .filter-pill.active { background: #f5f5f7; color: #0a0a0a; border-color: transparent; font-weight: 600; }
                ...
                .new-badge { position: absolute; top: 12px; left: 12px; background: linear-gradient(135deg, #0071e3, #4da3ff); color: #fff; font-size: 10px; font-weight: 700; padding: 4px 9px; border-radius: 6px; }
            ...
            <div className="page">
                ...
                <div className="hero">
                    <div className="hero-eyebrow">Just landed</div>
                    <h1 className="hero-title">New Arrivals ✨</h1>
                    <p className="hero-sub">The latest products freshly added to our store</p>
                </div>

                <div className="filters">
                    <div className="filters-inner">
                        {categories.map((cat) => (
                            <button key={cat.id} className={`filter-pill ${category === cat.id ? "active" : ""}`}
                                onClick={() => router.push(cat.id ? `/store/new-arrivals?category=${cat.id}` : "/store/new-arrivals")}>
                                {cat.label}
                            </button>
                        ))}
                    </div>
                ...
                <div className="container">
                    <div className="section-head">
                        <h2 className="section-title">{category ? `New in ${categories.find(c => c.id === category)?.label}` : "All new arrivals"}</h2>
                    ...
                    <div className="grid">
                        {loading ? (
                            ...
                            <div className="empty">
                                ...
                                <div className="empty-h">No new arrivals yet</div>
                                <div className="empty-p">Check back soon for the latest products</div>
                            </div>
                        ) : (
                            products.map((p) => (
...
export default function NewArrivalsPage() {
    return (
        <Suspense fallback={<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif", color: "rgba(255,255,255,0.4)", background: "#0a0a0a" }}>Loading...</div>}>
            <NewArrivalsContent />
        </Suspense>
    );
}